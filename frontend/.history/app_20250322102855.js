...
async function displayAssets() {
  try {
    ...
    assetsListDiv.innerHTML = ""; // Clear the current list
    ...
    userAssets.forEach((asset, index) => {
      ...
      // Convert the BigInt values using web3.utils.toBN() or toString()
      const valueInEther = web3.utils.fromWei(asset.value.toString(), "ether"); // Convert value to Ether
      const purchaseDate = new Date(
        parseInt(asset.purchaseDate) * 1000
      ).toLocaleDateString(); // Convert purchaseDate from Unix timestamp
      ...
      assetElement.innerHTML = `
        ...
        <p><strong>Image:</strong> <img src="${asset.imageURL}" alt="${
        asset.name
      }" width="100" /></p>
