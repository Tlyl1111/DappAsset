...
async function displayAssets() {
  try {
    ...
    userAssets.forEach((asset, index) => {
      ...
      // Convert the purchaseDate to a readable format
      const purchaseDate = new Date(
        parseInt(asset.purchaseDate) * 1000
      ).toLocaleDateString(); // Ensure purchaseDate is readable

      // Convert value from Wei to Ether
      const valueInEther = web3.utils.fromWei(asset.value.toString(), "ether");

      assetElement.innerHTML = `
        <h4>${asset.name}</h4>
        <p><strong>Purchase Date:</strong> ${purchaseDate}</p>
        <p><strong>Value:</strong> ${valueInEther} ETH</p>
...
async function viewAssetDetails(assetIndex) {
  try {
    ...
    const userAssets = await contract.methods
      .getUserAssets()
      .call({ from: userAccount });
    const assetId = userAssets[assetIndex]; // Get the correct asset ID
    ...
    // Convert purchaseDate to readable format
    const purchaseDate = new Date(
    ...
    assetInfoDiv.innerHTML = `
      ...
      <p><strong>Purchase Date:</strong> ${purchaseDate}</p>
      <p><strong>Value:</strong> ${valueInEther} ETH</p>
      <p><strong>Image:</strong> <img src="${assetDetails.imageURL}" alt="${assetDetails.name}" width="100" /></p>
      <p><strong>Note:</strong> ${assetDetails.note}</p>
    `;

    document.getElementById("assetDetails").style.display = "block";

    // Set the delete button to trigger the deletion of this asset
