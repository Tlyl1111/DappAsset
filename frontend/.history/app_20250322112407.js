...
// Listen for changes in search and filter fields
document.getElementById("searchKeyword").addEventListener("keyup", function () {
  searchKeyword = this.value.toLowerCase();
  displayAssets(); // Re-render the assets list based on new search criteria
});

document
  .getElementById("filterStartDate")
  .addEventListener("change", function () {
    startDateFilter = new Date(this.value).getTime() / 1000; // Convert to Unix timestamp (seconds)
    displayAssets();
  });
...
document
  ...
  .addEventListener("change", function () {
    endDateFilter = new Date(this.value).getTime() / 1000; // Convert to Unix timestamp (seconds)
    displayAssets();
  });
