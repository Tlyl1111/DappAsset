...
async function uploadImageToFirebase(file) {
  const storageRef = ref(storage, "assets/" + file.name); // Tham chiếu đến vị trí lưu trữ
  const uploadTask = uploadBytesResumable(storageRef, file); // Tải lên tệp

  try {
    await uploadTask; // Đợi cho việc tải lên hoàn thành
    const url = await getDownloadURL(storageRef); // Lấy URL của tệp đã tải lên
    return url;
  } catch (error) {
